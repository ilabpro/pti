...
- (IBAction)tryRegister:(id)sender {
    ...
    NSString *postString = [NSString stringWithFormat:@"cmd=register&fio=%@&company=%@&doljnost=%@&email=%@&login=%@&password=%@&password2=%@",fio.text,company.text,doljnost.text,email.text,login.text,pass_one.text,pass_two.text];
    ...
    hud.labelText = @"Регистрация. Ждите...";
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    if ([[[JSON objectForKey:@"success"] stringValue]  isEqual: @"1"])//
    {
        ...
        hud.labelText = @"Ожидайте модерации";
